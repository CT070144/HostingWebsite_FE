...
const Admin = () => {
  ...
  const menuItems = [
    ...
    },
    {
      ...
      path: '/admin/orders',
      subMenu: [
        { key: 'all-orders', label: 'Tất cả đơn hàng', path: '/admin/orders' },
        { key: 'pending-orders', label: 'Đơn hàng chờ xử lý', path: '/admin/orders/pending' },
        { key: 'completed-orders', label: 'Đơn hàng hoàn thành', path: '/admin/orders/completed' }
      ]
    },
    {
      key: 'products',
      ...
      path: '/admin/products',
      subMenu: [
        { key: 'all-products', label: 'Tất cả sản phẩm', path: '/admin/products' },
        { key: 'add-product', label: 'Thêm sản phẩm', path: '/admin/products/add' },
    ...
    
    {
      ...
      path: '/admin/reports',
      subMenu: [
        { key: 'sales-report', label: 'Báo cáo bán hàng', path: '/admin/reports/sales' },
        { key: 'revenue-report', label: 'Báo cáo doanh thu', path: '/admin/reports/revenue' },
        { key: 'user-report', label: 'Báo cáo người dùng', path: '/admin/reports/users' }
      ]
    },
    {
      key: 'settings',
      ...
      path: '/admin/settings',
      subMenu: [
        { key: 'general', label: 'Cài đặt chung', path: '/admin/settings/general' },
        { key: 'payment', label: 'Thanh toán', path: '/admin/settings/payment' },
        { key: 'email', label: 'Email', path: '/admin/settings/email' }
      ]
    }
  ];
  ...
  const orderTimeChartConfig = useMemo(() => {
    ...
    return {
      data: {
        labels: dashboardData.orderTimeData.map((item) => `${item.label} (${item.percentage}%)`),
        datasets: [
          {
  ...
  const orderStatusChartConfig = useMemo(() => {
    // Mock data cho chart trạng thái đơn hàng
        ...
        <div className="admin-content">
                ...
                <div className="stats-grid">
                  <div className="stat-card revenue">
                    ...
                    <div className="stat-content">
                      ...
                      <div className={`stat-change ${dashboardData.stats.revenueChange > 0 ? 'positive' : 'negative'}`}>
                        <i className={`fas fa-arrow-${dashboardData.stats.revenueChange > 0 ? 'up' : 'down'}`}></i>
                        {Math.abs(dashboardData.stats.revenueChange)}% so với tuần trước
                      </div>
                      <div className="stat-period">Doanh thu từ 1-12 Tháng 12, 2024</div>
                    </div>
                  ...
                  <div className="stat-card orders">
                    ...
                    <div className="stat-content">
                      ...
                      <div className="stat-value">{formatNumber(dashboardData.stats.totalOrders)}</div>
                      <div className={`stat-change ${dashboardData.stats.ordersChange > 0 ? 'positive' : 'negative'}`}>
                        <i className={`fas fa-arrow-${dashboardData.stats.ordersChange > 0 ? 'up' : 'down'}`}></i>
                        {Math.abs(dashboardData.stats.ordersChange)}% so với tuần trước
                      </div>
                      <div className="stat-period">Đơn hàng từ 1-6 Tháng 12, 2024</div>
                    </div>
                    <button className="view-report-btn">Xem báo cáo</button>
                  </div>
                  ...
                  <div className="stat-card products">
                    ...
                    <div className="stat-content">
                      ...
                      <div className="stat-value">{formatNumber(dashboardData.stats.totalProducts)}</div>
                      <div className={`stat-change ${dashboardData.stats.productsChange > 0 ? 'positive' : 'negative'}`}>
                        <i className={`fas fa-arrow-${dashboardData.stats.productsChange > 0 ? 'up' : 'down'}`}></i>
          ...
          {activeSection !== 'dashboard' && (
            <div className="section-placeholder">
              <h1 className="page-title">{menuItems.find(m => m.key === activeSection)?.label || 'Section'}</h1>
              <p>Nội dung cho {menuItems.find(m => m.key === activeSection)?.label || 'section'} sẽ được thêm sau.</p>
            </div>
          )}
        </div>
      </main>
